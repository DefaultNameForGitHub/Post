...
class Ui_MainWindow(object):
    ...
    def retranslateUi(self, MainWindow):
        ...
        self.pushButton_3.setText(_translate("MainWindow", "Noise"))
...
class mywindow(QtWidgets.QMainWindow):
    ...
    # код метода представления амплитуд МШУ в графическом виде
    def Noise_btn(self):
    ...
    def btn_1Clicked(self):
        ...
        if 0 < response_list_0.rtt_avg_ms < 2 and 0 < response_list_1.rtt_avg_ms < 2:
            self.ui.text = 'Подключение установлено.\n'
            self.ui.text += ('Время пинга: \nPV-4201 = ' +
                             str(response_list_0.rtt_avg_ms) +
                             ' мс;\n' +
                             'PV-4202 = ' +
                             str(response_list_1.rtt_avg_ms) +
                             ' мс.\n')
            self.ui.text += 'Нажмите кнопку Старт...\n'
            self.ui.pushButton.setEnabled(True)
            self.ui.response = True
    ...
    # метод получения амплитуд МШУ
    def btnClicked(self):

        # деактивация кнопок при работе программы
        self.ui.pushButton.setEnabled(False)
        ...
        if self.ui.response == True:
            ...
            # длина поссылки
            band_len = {'D2': 488, 'D3': 488, 'D4': 1448, 'D5': 1448}
            # ip адрес
            ...
            for band in bands:
                ...
                while True:
                    ...
                    if len(data) == band_len[band]:
                        ...
                        data = list(
                            struct.unpack(
                                '<' + 'H' * int(band_len[band]/2),
                                data))
                    else:
                        # ждем ответа дальше (пока не придет), если не пришел пакет
                        # длительностью band_len[band]
                        continue
